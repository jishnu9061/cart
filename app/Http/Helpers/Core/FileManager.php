...
//        Storage::copy($from, Storage::disk(self::STORAGE_DISK)->getDriver()->getAdapter()->getPathPrefix() . $to);
     ...
     * @return string
     */
    public static function getFileUrl(string $fileName, string $destination): string
    {
        return Storage::disk('public')->url($destination . $fileName);
    }
    ...
    {
        return Storage::disk(self::STORAGE_DISK)->path($destination . $fileName);
    }
    ...
    {
        $status = true;
        $rule = 'required|file|mimes:xlsx,xls,csv,jpg,jpeg,png,bmp,doc,docx,pdf,tif,tiff,txt,rtf,svg';
        if (self::FILE_TYPE_EXCEL == $type) {
        ...
        } elseif (self::FILE_TYPE_COMMON_MUTI_FILE == $type) {
            $validator = Validator::make(Request::all(), [
                $fileName . '.*' => 'mimes:doc,pdf,docx,zip,txt,jpg,jpeg,png,xlsx,xls,csv,tif,tiff,rtf,svg',
            ]);
            if ($validator->fails()) {
                $status = false;
            }
            ...
            if (self::FILE_TYPE_IMAGE == $type) {
                $rule = 'required|file|mimes:jpg,jpeg,png,bmp,svg';
            ...
            } elseif (self::FILE_TYPE_VIDEO == $type) {
                $rule = 'required|file';
            }
            $validator = Validator::make(Request::all(), [
                $fileName => $rule,
    ...
    public static function createDirectory(String $path)
    {
     ...
     public static function checkFileExist(string $fileName, string $destination): bool
    {
        return Storage::disk('public')->exists($destination . '/' . $fileName);
    }
